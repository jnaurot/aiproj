<script lang="ts">
	...
	$: downstreamSuggestion = buildDownstreamSchemaSuggestion({
		mismatchMessage,
		sourceSchema: sourceSchemaRaw,
		targetSchema: targetSchemaRaw,
		targetLabel
	});
	$: downstreamSuggestionText = downstreamSuggestion
		? JSON.stringify(downstreamSuggestion.targetSchema, null, 2)
		: '';
	...
	function applySource() {
		...
		applySourceResult = (graphStore as any).updateNodeSchema?.(sourceNodeId, sourceHandleId, 'output', parsed) ?? {
			ok: false,
			error: 'updateNodeSchema not available'
		};
	}
	...
	function applyTarget() {
		...
		applyTargetResult = (graphStore as any).updateNodeSchema?.(targetNodeId, targetHandleId, 'input', parsed) ?? {
			ok: false,
			error: 'updateNodeSchema not available'
		};
	}

	function useSuggestedTargetSchema() {
		if (!downstreamSuggestion) return;
		targetDraft = JSON.stringify(downstreamSuggestion.targetSchema, null, 2);
		targetParseError = null;
		applyTargetResult = null;
	}
...
>
	<div class="sei-header">
		...
		<button class="sei-close" on:click={onClose} aria-label="Close inspector" title="Close">x</button>
	</div>

	{#if mismatchMessage && snapshot?.effectiveSeverity !== 'clean'}
		<div class={`sei-banner sei-banner--${snapshot?.effectiveSeverity ?? 'warning'}`}>
			{#if snapshot?.effectiveSeverity === 'error'}!{:else}i{/if}
			{mismatchMessage}
		</div>
		{#if downstreamSuggestion}
			<div class="sei-suggestion">
				<div><strong>Suggestion:</strong> {downstreamSuggestion.summary}</div>
				<pre class="sei-suggestion-schema">{downstreamSuggestionText}</pre>
				{#if targetIsEditable}
					<button class="sei-apply-btn" on:click={useSuggestedTargetSchema}>Use suggested target schema</button>
				{/if}
			</div>
		{/if}
