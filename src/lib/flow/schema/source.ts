...
export const SourceFileParamsSchema = z
	.object({
		...
		quote_char: z.string().min(1).max(1).optional(),
		escape_char: z.string().min(1).max(1).optional(),
		malformed_row_policy: z.enum(["fail", "skip", "warn"]).optional(),
		...
		txt_record_mode: z.enum(["raw", "lines", "paragraphs", "fixed_chunk"]).optional(),
		txt_chunk_size: z.number().int().positive().optional(),
		pdf_extraction_mode: z.enum(["text", "ocr", "tables", "hybrid"]).optional(),
		image_extract_metadata: z.boolean().optional(),
		image_svg_policy: z.enum(["pass", "sanitize", "reject"]).optional(),
		image_tiff_pages_mode: z.enum(["first", "all"]).optional(),
		audio_extract_metadata: z.boolean().optional(),
		audio_normalize: z.boolean().optional(),
		audio_target_peak: z.number().min(0.01).max(1).optional(),
		audio_transcode_format: z.enum(["mp3", "wav", "flac", "ogg", "m4a", "aac"]).optional(),
		video_extract_metadata: z.boolean().optional(),
		video_frame_mode: z.enum(["none", "keyframes", "interval"]).optional(),
		video_frame_interval_sec: z.number().positive().optional(),
		video_max_frames: z.number().int().positive().optional(),
