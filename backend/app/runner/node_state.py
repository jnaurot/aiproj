...
def canonical_cache_schema_view(*, node_kind: str, raw_schema: Any) -> Dict[str, Any]:
    ...
    has_channels = any(
        k in stripped
        for k in ("expectedInputSchema", "expectedInputSchemas", "expectedSchema", "inferredSchema", "observedSchema")
    )
    if not has_channels:
    ...
    allowed_channels = ("expectedInputSchema", "expectedInputSchemas", "expectedSchema", "inferredSchema")
    view: Dict[str, Any] = {}
...
def _schema_declared_type_from_cache_view(schema_view: Dict[str, Any]) -> Optional[str]:
    if not isinstance(schema_view, dict):
        return None
    for key in ("expectedSchema", "inferredSchema", "expectedInputSchema", "expectedInputSchemas"):
